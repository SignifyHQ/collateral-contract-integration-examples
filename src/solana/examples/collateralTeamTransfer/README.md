...
A collateral account is like a **digital vault** on the Solana blockchain that:
- Holds valuable assets (tokens, funds, etc.)
- Has **admin controls** - only specific people can manage it
- Requires **cryptographic signatures** to make changes (like a digital signature on important documents)
...
The script follows a **secure 2-step process**:

1. **Step 1 - Create Signatures**: Current admin creates cryptographic proof that they authorize the transfer
2. **Step 2 - Execute Transfer**: The blockchain verifies the signatures and transfers control to the new admin

This ensures that **only the legitimate current admin** can transfer control, and everything is permanently recorded on the blockchain.
...
You need to know these 3 pieces of information:

| Parameter | Description | How to Get It |
|-----------|-------------|---------------|
| `collateralAddress` | The address of the collateral account you want to transfer | This should be provided by whoever created the collateral account |
| `programId` | The Solana program that manages this collateral | Usually provided along with the collateral address |
...
If you know your collateral address, you can look up the program ID using Solana Explorer:

1. **Go to Solana Explorer**: https://explorer.solana.com/
2. **Switch to the correct network** (Mainnet Beta or Devnet)
3. **Search for your collateral address** in the search bar
4. **Find the "Assigned Program Id"** field - this is your `programId`

**Example:**
- Collateral Address: `5ZPYhy56gKHoG8SuydzTv1eVnHCTggPmHqUx51P7QwN7`  
- Explorer URL: https://explorer.solana.com/address/5ZPYhy56gKHoG8SuydzTv1eVnHCTggPmHqUx51P7QwN7?cluster=devnet
- Look for **"Assigned Program Id"** field to get your `programId`
...
The collateral 7xKXtg2CW87d97TX5jBkheTqA83TZRuJosgAsU is now administered by 3WdDdKJaZmn2QDTWR8xVzjdHLZQmqzjdwMNXyz
```
...
- **Backup**: Make sure the new admin has secure backup of their private key
...
For more details, see the [Solana Test Validator Guide](https://solana.com/es/developers/guides/getstarted/solana-test-validator).
...
Execute the collateral transfer using your local environment:
