 ...
 * The withdrawal process is implemented as a custom program instruction that allows a
 * self-managed tenant to retrieve funds from its collateral account to a personal wallet.
 *
 * This is the SELF-MANAGED TENANT variant of ../multisig_program_v2_01: identical on-chain flow,
 ...
 * }
 * 
 * Success Response (Tenant Contracts):
 * [
 *   {
 ...
 * 
 * Command Line Usage:
 * node dist/src/solana/examples/withdrawal/multisig_program_v2_01_tenant/index.js token amount adminAddress recipientAddress chainId
 *
 * Arguments:
 ...
 * - chainId: The identifier for the Solana chain/network (e.g., "900" for mainnet; the signature domain is pinned to chain 900)
 *
 * Flow & Expectations:
...
const BASE_URL = process.env.RAIN_API_URL || "https://api.raincards.xyz";
 ...
 */
function getProgram(programAddress: string, signer: Keypair): Program<MainV2_01> {
  ...
  return new Program<MainV2_01>(idl, provider)
}
...
const main = async ({
  ...
  //build API request
  const signatureUrl = `${BASE_URL}/v1/issuing/tenants/signatures/withdrawals`;
  const params = {
    token,
    amount,
    adminAddress,
  ...
  //build API request
  const contractsUrl = `${BASE_URL}/v1/issuing/tenants/contracts`;
...
// Param & header
const token = process.argv[2]; // token to withdraw
const amount = process.argv[3]; // amount of token to withdraw (in cents)
const adminAddress = process.argv[4]; // should be admin on collateral contract
...
if (!token || !amount || !adminAddress || !recipientAddress || !chainId) {
  throw new Error("Required query parameters not defined");
}
main({ token, amount, adminAddress, recipientAddress, chainId }).catch((ex) =>
  console.error(ex)
);
