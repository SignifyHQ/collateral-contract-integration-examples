 ...
 * 
 * Command Line Usage:
 * node dist/examples/withdrawal_v2_2/index.js userId token amount ownerAddress recipientAddress chainId
 * 
 * Arguments:
...
const token = process.argv[3]; // token to withdraw (or PublicKey.default for SOL)
const amount = process.argv[4]; // amount of token to withdraw
const ownerAddress = process.argv[5]; // must be owner of single-signer collateral
...
if (!userId || !token || !amount || !ownerAddress || !recipientAddress || !chainId) {
  throw new Error("Required query parameters not defined");
}

main({ userId, token, amount, ownerAddress, recipientAddress, chainId }).catch((ex) =>
  console.error(ex)
);
